...
export DISPLAY=:99
sleep 1
ACTIVATE_JCO="${ACTIVATE_JCO:-0}"
JCO_NAME="${JCO_NAME:-SIN_JCO}"
...
echo "ACTIVATE_JCO: $ACTIVATE_JCO"
echo "JCO_NAME: $JCO_NAME"
echo "----------------------------------------"
echo "----------------------------------------"
...
CMD="$EJECUTABLE_MASTER $NOMBRE_MODELO_PEST $COMANDO_MASTER :$PEST_PORT"
...
if [ "$ACTIVATE_JCO" = "1" ]; then
    echo "JCO activado: se enviara '$JCO_NAME' como entrada al comando."
    RUN_CMD() { echo "$JCO_NAME" | $CMD; }
else
    RUN_CMD() { $CMD; }
fi
