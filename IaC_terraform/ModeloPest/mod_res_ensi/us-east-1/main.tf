...
############################################
# SG del nodo central: permite 4004/TCP e ICMP desde VPC east1 (solo)
...
# Inbound 4004 desde CIDR de la VPC east1 (quitar reglas que apuntaban a east1/east2)
...
# Inbound ICMP (ping) desde CIDR de la VPC east1 (quitar reglas east1/east2)
...
# Security Group para VPC Endpoints Interface (ECR API y ECR DKR) - mantiene en east1
resource "aws_security_group" "endpoint" {
  ...
  ingress {
    ...
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = [aws_vpc.east1.cidr_block]  # todo el rango de la VPC
  }
...
# SG para hosts en east1 (mantener)
...
resource "aws_instance" "central" {
  ...
  CWAGENTCFG
    ...
    docker run -d --name pest-central \
      ...
      --log-driver=awslogs \
      --log-opt awslogs-region=${var.region_east1} \
...
resource "aws_appautoscaling_target" "host_east1" {
  max_capacity       = 1
  min_capacity       = 1
  resource_id        = "service/${aws_ecs_cluster.east1.name}/${aws_ecs_service.host_east1.name}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
}

resource "aws_appautoscaling_policy" "host_east1_cpu" {
  name               = "${var.project_name}-host-cpu-target"
  service_namespace  = "ecs"
  resource_id        = aws_appautoscaling_target.host_east1.resource_id
  scalable_dimension = aws_appautoscaling_target.host_east1.scalable_dimension
