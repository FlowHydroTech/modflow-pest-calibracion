    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:s3:::${var.s3_bucket_talabre}",
          "arn:aws:s3:::${var.s3_bucket_talabre}/*"
        ]
      }
    ]
...
resource "aws_ecs_cluster" "cluster-pest-autonomo-talabre" {
...
resource "aws_ecs_service" "master_service" {
  ...
  cluster                = aws_ecs_cluster.cluster-pest-autonomo-talabre.id
...
resource "aws_ecs_task_definition" "task-pest-talabre-autonomo" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
        ...
        { name = "BUCKET_NAME", value = var.s3_bucket_talabre },
...
resource "aws_ecs_task_definition" "task-pest-talabre-master" {
  ...
  container_definitions = jsonencode([{
    name      = "${var.project_name}-master-container"
    image     = var.ecr_image_talabre
    essential = true
...
aws ecs run-task --cluster ${aws_ecs_cluster.cluster-pest-autonomo-talabre.name} --launch-type FARGATE --task-definition ${aws_ecs_task_definition.task-pest-talabre-agente.arn} --count ${var.agent_count} --enable-execute-command --network-configuration file://$path
